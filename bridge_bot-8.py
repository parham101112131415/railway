...
def _reel_menu_title(url):
    """عنوان منوی ریلز/تیک‌تاک."""
    if dl.is_tiktok_url(url or ""):
        return "🎵 تیک‌تاک — انتخاب کن:"
    return "🎵 اینستاگرام — انتخاب کن:"


def build_ig_action_kb(batch=False, movie=True):
    """منوی اینستا/تیک‌تاک — تک‌لینک و چندلینک یکسان.
    movie=False برای تیک‌تاک: شناسایی فیلم فقط با دانلودر اینستاگرام کار می‌کنه."""
    kb = [[InlineKeyboardButton("🎵 استخراج آهنگ", callback_data="ig_audio")]]
    if movie:
        kb.append([InlineKeyboardButton("🎬 شناسایی فیلم", callback_data="ig_movie")])
    kb += [
...
_GENERIC_SOUND_RE = re.compile(
    r"original\s*(sound|audio)|orijinal\s*ses|son\s*original|sonido\s*original|"
    r"suono\s*originale|som\s*original|originalton|оригинальн\w*\s*(звук|аудио)|"
    r"صدای?\s*اصلی|الصوت\s*الأصلي|原声|オリジナル",
    re.I,
)


def _norm_name(s):
    """اسم/آیدی رو برای مقایسه نرمال می‌کنه (حروف کوچک، بدون فاصله و علامت)."""
    return re.sub(r"[\W_]+", "", str(s or "").lower())


def _is_tiktok_inf(inf):
    ext = (inf.get("extractor_key") or inf.get("extractor") or "").lower()
    if "tiktok" in ext:
        return True
    return "tiktok.com" in (inf.get("webpage_url") or inf.get("original_url") or "").lower()


def _tiktok_music_lookup_sync(url):
    """
    اسم آهنگ/خواننده‌ای که خودِ تیک‌تاک برای این ویدیو ثبت کرده رو از JSON داخل صفحه
    می‌خونه (همون چیزی که بقیه‌ی رباتا نشون می‌دن). yt-dlp همیشه این فیلد رو
    برنمی‌گردونه، برای همین مستقیم می‌خونیمش.
    برمی‌گردونه dict با کلیدهای title / artist / original / account_ids یا None.
    """
    try:
        import curl_cffi.requests as _cr
    except Exception as e:
        print(f"[tiktok-page] curl_cffi نیست: {e}", flush=True)
        return None
    try:
        s = _cr.Session(impersonate="chrome")
        r = s.get(
        ...
        print(f"[tiktok-page] دریافت صفحه ناموفق: {type(e).__name__}: {e}", flush=True)
        return None
    ...
    if not item:
        # فرمت قدیمی‌تر صفحه
        m2 = re.search(r'<script[^>]+id="SIGI_STATE"[^>]*>(.*?)</script>', html, re.S)
        if m2:
            try:
                sigi = json.loads(m2.group(1))
                mods = sigi.get("ItemModule") or {}
                if mods:
                    item = next(iter(mods.values())) or {}
            except Exception:
                item = {}
    music = (item or {}).get("music") or {}
    print(
        f"[tiktok-page] status={getattr(r, 'status_code', '?')} html={len(html)}B "
        f"json={'yes' if data else 'no'} music_title={music.get('title')!r} "
        f"music_author={music.get('authorName')!r} original={music.get('original')!r}",
        flush=True,
    )
    ...
    if not isinstance(author, dict):
        author = {"uniqueId": str(author)}
    return {
        "title": title,
        "artist": (music.get("authorName") or "").strip(),
        "original": bool(music.get("original")),
        "account_ids": [
            x for x in (author.get("uniqueId"), author.get("nickname")) if x
        ],
    }
...
def extract_song_query(inf):
    ...
    if not artist:
        _arts = inf.get("artists")
        if isinstance(_arts, (list, tuple)):
            artist = ", ".join(str(a).strip() for a in _arts if a).strip()

    if _is_tiktok_inf(inf):
        print(
            f"[tiktok-meta] track={track!r} artist={artist!r} "
            f"uploader={inf.get('uploader')!r} uploader_id={inf.get('uploader_id')!r}",
            flush=True,
        )
        # تیک‌تاک: برای «original sound» متادیتا اسم خودِ اکانت آپلودکننده‌ست، نه آهنگ.
        # پس اگه track/artist مثل اسم اکانت یا «صدای اصلی» بود دورش می‌ندازیم و
        # می‌ره سراغ تشخیص از روی صدا (AudD / شزام / ACRCloud) — مثل اینستا.
        accounts = set()
        for k in ("uploader", "uploader_id", "channel", "creator", "channel_id"):
            v = _norm_name(inf.get(k))
            if v:
                accounts.add(v)

        def _looks_like_account(val):
            n = _norm_name(val)
            if not n:
                return False
            for a in accounts:
                if n == a or (len(a) >= 4 and a in n) or (len(n) >= 4 and n in a):
                    return True
            return False

        if track and (_GENERIC_SOUND_RE.search(track) or _looks_like_account(track)):
            track = ""
        if artist and _looks_like_account(artist):
            artist = ""
        if track and artist:
            return f"{artist} {track}"
        if track:
            return track
        # کپشن تیک‌تاک هیچ‌وقت اسم آهنگ نیست → حدس نزن، از روی صدا تشخیص بده
        return None
...
async def search_and_send_music_options(chat_id, ctx, search_query, cb_prefix, result_key, update=None, auto_pick=False, artist=None, title=None, extra_queries=None, count=5, silent_not_found=False):
    """
    ...
    if not entries:
        print(f"[music] یوتیوب هیچ نتیجه‌ای نداد برای: {search_query!r} (extra={extra_queries!r})", flush=True)
        if silent_not_found:
            # صدا زننده خودش تصمیم می‌گیره (مثلاً می‌ره سراغ تشخیص از روی صدا)
            try:
                await msg.delete()
            except Exception:
                pass
...
async def start_single(update, ctx, url):
    ...
    if dl.is_reel_url(url):
        await tg_call(
            msg.edit_text,
            build_meta_text(inf) + "\n\n" + _reel_menu_title(url),
            reply_markup=InlineKeyboardMarkup(build_ig_action_kb(batch=False, movie=dl.is_instagram_url(url or ""))),
        )
        return
...
async def process_next_in_batch(update, ctx, edit_msg=None):
    ...
    if dl.is_reel_url(url):
        await tg_call(
            ...
            reply_markup=InlineKeyboardMarkup(build_ig_action_kb(batch=True, movie=dl.is_instagram_url(url or ""))),
        )
    else:
...
async def execute_ig_song_extract(update, ctx, url, inf):
    ...
    try:
        ...
        if not search_query and dl.is_tiktok_url(url):
            # yt-dlp آهنگ تیک‌تاک رو نداد → مستقیم از صفحه‌ی تیک‌تاک بخون
            try:
                _tm = await asyncio.to_thread(_tiktok_music_lookup_sync, url)
            except Exception as _e:
                print(f"[tiktok-page] خطا: {type(_e).__name__}: {_e}", flush=True)
                _tm = None
            if _tm:
                _inf2 = dict(inf or {})
                _inf2["track"] = _tm["title"]
                _inf2["artist"] = _tm["artist"]
                _inf2.pop("artists", None)
                if _tm.get("account_ids"):
                    _inf2["uploader_id"] = _inf2.get("uploader_id") or _tm["account_ids"][0]
                    _inf2["creator"] = _inf2.get("creator") or _tm["account_ids"][-1]
                if not _tm.get("original"):
                    search_query = extract_song_query(_inf2)
                print(f"[tiktok-page] query={search_query!r}", flush=True)
        if search_query:
            _res = await search_and_send_music_options(
            ...
            if _res != "NOT_FOUND":
                return True
            print(f"[song-id] متادیتا ({search_query!r}) نتیجه نداد → تشخیص از روی صدا", flush=True)
...
async def run_single_job(update, ctx, job, queue_info=None):
    ...
    if path:
        ...
        try:
            is_ig = bool(job.get("ig_kind")) or bool(url and dl.is_reel_url(url))
            if path and os.path.exists(path) and is_ig:
                ...
                if kind == "reel" or (url and dl.is_reel_url(url) and not job.get("is_audio")):
                    job["send_to_telegram"] = True
            else:
                print(
...
async def button(update, ctx):
    ...
    if data.startswith("bulk:"):
        ...
        if action == "back":
            ...
            if url and dl.is_reel_url(url):
                kb = build_ig_action_kb(batch=is_batch, movie=dl.is_instagram_url(url or ""))
            else:
    ...
    if data in ("pref:yes", "pref:no"):
        ...
        if s.get("mode") == "batch" or s.get("links"):
            ...
            if dl.is_reel_url(url or ""):
                await tg_call(
                    ctx.bot.send_message,
                    chat_id,
                    nlabel + build_meta_text(inf or {}) + "\n\n" + _reel_menu_title(url or ""),
                    reply_markup=InlineKeyboardMarkup(build_ig_action_kb(batch=True, movie=dl.is_instagram_url(url or ""))),
                )
            else:
    ...
    if data == "qgo":
        ...
        for key in sel:
            ...
            if base_url and dl.is_reel_url(base_url):
                job["send_to_telegram"] = True
    ...
    if data.startswith("q:"):
        ...
        if sel == "best":
            ...
            job["fmt"] = sel
        is_ig = bool(base_url and dl.is_reel_url(base_url))
        if is_ig:
            job["send_to_telegram"] = True
            job["ig_kind"] = "song" if job["is_audio"] else "reel"
        try:
